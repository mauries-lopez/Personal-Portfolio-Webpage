...
export const MobileMenu = ({curHamburgerStatus, setCurHamburgerStatus, overflowStatus, setOverflowStatus}) => {
    ...
    return (
        <div className="fixed top-0 left-0 z-50 h-dvh w-dvw flex flex-row">
            <div id="mainMenu" className={`relative h-dvh z-20 3xs:w-[100%] md:w-[50%] bg-white text-black/95 flex flex-col ${animateClass} bg-no-repeat bg-cover bg-center`} onWheel={handleHamburgerClick}>
                <div className="absolute h-full w-full blur-[2px] bg-black" style={{backgroundImage: "url(/mainMenuBG.png"}} />
                ...
                <div id="middlePortion" className="relative h-[60%] w-full flex justify-center items-start" >
                    <div>
                        <ul className="list-inside list-[upper-roman] text-3xl/17 font-bold">
                            ...
                            <Link to="experience" spy={true} smooth={true} offset={window.innerWidth < 640 ? -50 : -80} duration={800} onClick={() => {handleHamburgerClick()}}>
                                <li className="hover:underline cursor-pointer"> Experience </li>
                            </Link>

                            <Link to="project0" spy={true} smooth={true} offset={window.innerWidth < 640 ? -30 : 0} duration={800} onClick={() => {handleHamburgerClick()}}>
