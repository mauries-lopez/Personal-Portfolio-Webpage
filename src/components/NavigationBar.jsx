...
export const NavigationBar = ({ curHamburgerStatus, setCurHamburgerStatus, overflowStatus, setOverflowStatus, navTitle}) => {
    ...
    return (
        <nav className="fixed top-0 z-40 w-dvw md:h-[6rem] xs:h-[4rem] 3xs:h-[4rem] flex flex-row items-center" style={{backgroundColor: "#1A1A1A"}}>
            ...
            <div className="relative h-full w-full" id="titlePartition">
                <div className="h-full w-full flex flex-row items-center md:justify-start 3xs:justify-center md:text-4xl sm:text-3xl 3xs:text-lg font-bold text-white text-center">
                    <span > {navTitle} </span>
                </div>
            </div>
