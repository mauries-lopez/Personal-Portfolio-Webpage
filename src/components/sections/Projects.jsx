...
export const Projects = ({
  ...
  return (
    <section
      id={projectId}
      ref={setProjectNode}
      className='relative h-lvh w-svw justify-center items-center bg-[#F0F0F0]'
    >
      <div
        className={`h-full w-full flex 3xs:flex-col ${reverse ? 'md:flex-row-reverse' : 'md:flex-row'} justify-center items-center`}
      >
        <div className='sm:h-full 3xs:h-[50%] w-full flex  justify-center items-center p-3 mt-3 z-10'>
          <div className='h-full w-[80%] flex flex-col justify-center items-start'>
            <p className='md:text-9xl 3xs:text-2xl font-sans font-bold pointer-events-none'>
              {number}
            </p>
            <a
              href={`${hrefFile}`}
              download={`${hrefDownloadName}`}
              target='_blank'
              className='animate-pulse' rel='noreferrer'
            >
              <p className='md:text-6xl 3xs:text-xl font-sans font-bold mt-5'>
                {title}
              </p>
            </a>
            ...
            <p className='md:text-xl 3xs:text-md font-sans mt-2 mb-3 overflow-y-auto scroll-shadows'>
              {description}
            </p>
          </div>
        </div>
        ...
        <div
          id='image0'
          className='relative md:flex md:h-full 3xs:h-[40%] w-full 3xs:w-full md:mask-radial-from-99% md:mask-radial-[40%_40%] overflow-hidden z-10'
          onMouseEnter={learnMoreShow}
          onMouseLeave={learnMoreHide}
        >
          <div
            className={`absolute bg-fixed h-full w-full duration-500 bg-cover scale-100 bg-no-repeat ${learnMoreBool ? 'blur-xs' : ''}`}
          ...
          >
            <div
              className={`absolute cursor-pointer flex flex-col justify-center items-center h-full w-full bg-[#F0F0F0]/60 duration-500 3xs:opacity-100 ${learnMoreBool ? 'md:opacity-100' : 'md:opacity-0'}`}
            >
              <p className='text-5xl font-bold pt-3 text-[#1A1A1A]'>
                {hrefText}
              </p>
              <p className='text-[0.7rem] italic pt-3 text-[#1A1A1A]'>
        ...
        <div className='3xs:absolute md:hidden h-full w-full'>
          <div
            className={`${project1Bool || project2Bool || project3Bool || project4Bool || project5Bool || project6Bool ? 'opacity-100' : 'opacity-0'} duration-3000 h-full w-full flex items-center justify-start`}
            style={{
              position: 'absolute',
              bottom: 0,
              left: 0,
